...
9. **Evidence precedes detailed questions.** The analyst owns task, domain, and background research, including motivation, problem, beneficiaries, outcomes, and reasonable user or operational expectations. Sources, findings, confidence, assumptions, and provenance are recorded, and observed or domain expectations remain distinct from explicit human requirements.
10. **Documentation tags are bounded.** After applicable quality and baseline gates, only the analyst may create an immutable tag in the configured documentation repository, normally `spec-YYMMDD` with a deterministic collision suffix. Tags are recorded with repository, tag, commit, baseline, validation, and publication status; they are never moved, deleted, or repointed. Tag creation is separate from push/publication authorization, and no source, release, deployment, or production tags are created by the analyst.
