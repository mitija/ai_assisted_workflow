...
| [`conductor`](agent/conductor.md) | Orchestrates multi-step work end to end through its thorough six-phase workflow: Analyze, Code or non-code Decompose, Execute, Review, Escalate when needed, and Report. Runs on a better AI model than sub-agents — owns the thinking, planning, and decision-making. Interactive by default for ambiguity resolution; autonomous when requested. | Primary |
